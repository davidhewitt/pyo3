...
Because Python objects are freely shared between threads by the Python interpreter, all structs annotated with `#[pyclass]` must implement both `Send` and `Sync`.
...
#[pyclass]
struct MyIterator {
    iter: Box<Iterator<Item = PyObject> + Send + Sync>,
}
