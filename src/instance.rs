...
/// ```
/// use pyo3::ffi;
/// use pyo3::{ToPyObject, AsPyPointer, PyNativeType, IntoPyValue};
/// use pyo3::types::{PyDict, PyAny};
///
/// pub fn get_dict_item<'p>(dict: &'p PyDict, key: impl IntoPyValue) -> Option<&'p PyAny> {
///     let key = key.into_managed_py_ref(dict.py());
///     unsafe {
...
impl<'p, T> ManagedPyRef<'p, T> {
    pub fn from_owned(py: Python<'p>, data: T) -> Self
    where T: IntoPyValue<'p>
    {
        Self::_new(py, data.into_py_value(py).into_ptr(), true)
    }

    pub fn from_borrowed(py: Python<'p>, data: T) -> Self
    where T: AsPyPointer
    {
        Self::_new(py, data.as_ptr(), false)
    }
...
impl<'p, T> Drop for ManagedPyRef<'p, T> {
    ...
    fn drop(&mut self) {
        if self.owned {
            // unsafe { panic!("SCARY {}", ffi::Py_REFCNT(self.data)); }
...
mod test {
    ...
    fn borrowed_py_ref_with_to_pointer() {
        ...
        let borrowed = ManagedPyRef::from_borrowed(py, native);
    ...
    fn borrowed_py_ref_with_to_object() {
        ...
        let borrowed = ManagedPyRef::from_owned(py, convertible);
