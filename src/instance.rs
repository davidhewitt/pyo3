...
impl<'py> Bound<'py, PyAny> {
    ...
    ///
    /// - `ptr` must be a valid pointer to a Python object.
    /// - `ptr` must own the Python reference
    pub(crate) unsafe fn from_ptr_unchecked(py: Python<'py>, ptr: *mut ffi::PyObject) -> Self {
        Self(
            py,
            ManuallyDrop::new(Py::from_non_null(NonNull::new_unchecked(ptr))),
        )
    }
}
    ...
    T: PyClass,
{
    ...
    /// Provide an immutable borrow of the value `T` without doing any runtime
    /// borrow checking.
    ///
